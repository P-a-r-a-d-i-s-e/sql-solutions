...
/* Find Category Recommendation Pairs */
    WITH user_with_category (user_id, category) AS
         (SELECT DISTINCT user_id, category
            FROM ProductPurchases pp
                 JOIN ProductInfo pi
                 ON pp.product_id = pi.product_id
         )
  SELECT uwc1.category category1, uwc2.category category2, COUNT(1) customer_count
    FROM user_with_category uwc1
         JOIN user_with_category uwc2
              ON uwc1.category < uwc2.category
                 AND uwc1.user_id = uwc2.user_id
